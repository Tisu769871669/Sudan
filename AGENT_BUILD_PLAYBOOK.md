# OpenClaw 客服 Agent 制作流程与步骤

本文档用于复刻当前苏丹客服 Agent 的制作方式，也适用于在另一台 OpenClaw 服务器上把 `main` agent 改造成新的目标业务客服 Agent。
...
```text
请在另一台 OpenClaw 服务器上复刻我们当前苏丹客服 agent 的制作流程，但目标不是创建独立 agent，而是把该服务器的 main agent 改造成目标业务 agent。
